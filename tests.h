...
#else
  const mpi_info_struc mpi_info = mpi_info_null;
#endif
...
inline void my_print(std::string text, int rank, int rank_to_write=0){
/** \brief Write output
*
...
inline void my_print(std::fstream * handle, std::string text, int rank, int rank_to_write=0){
/** \brief Write output
*
* Currently dump to term. Perhaps also to log file. Accomodates MPI also. Set rank_to_write to -1 to dump from all. Default value is 0
*/
...
inline void my_print(std::string text){
    std::cout<< text<<std::endl;
}
...
*/
  ...
  void add_test(test_entity* test){test_list.push_back(test);};
  ...
  */
  ...
  void set_colour(char col=0){my_print(this->get_color_escape(col));};
